...
class StockPicking(models.Model):
    ...
    def _generate_postlogistics_label(
        ...
        # To be able to properly test this bahavior, we can't use the current
        # cursor, as we need to rollback the transaction if there is an error
        # Odoo does not allow to rollback or commit inside a test instance
        with Registry(self.env.cr.dbname).cursor() as new_cr:
            self_new_env = self.with_env(
            ...
            failed_label_results = [
                label for label in label_results if "errors" in label
            ]

            # Case when there is a failed label, rollback odoo data
            if failed_label_results:
                self_new_env._cr.rollback()
                self_new_env = self_new_env.exists()

            labels = self_new_env.write_tracking_number_label(
                success_label_results, packages
            )

            if not skip_attach_file:
                for label in labels:
                    self_new_env.attach_shipping_label(label)

            if failed_label_results:
                # Commit the change to save the changes,
                # This ensures the label pushed recored correctly in Odoo
                self_new_env._cr.commit()  # pylint: disable=invalid-commit
                error_message = "\n".join(
                    self_new_env._cleanup_error_message(label["errors"])
                    for label in failed_label_results
                )
                raise UserError(
                    self_new_env.env._("PostLogistics error:") + "\n\n" + error_message
                )
        return labels
